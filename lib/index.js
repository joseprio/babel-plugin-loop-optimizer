    ...
    visitor: {
      ...
      CallExpression: function CallExpression(path) {
        ...
        if (checkName(path) && path.node.arguments.length === 1 && t.isExpression(path.node.arguments[0].body)) {
          ...
          var forBodyDeclarations = [t.variableDeclarator(originalExpression.params[0].name, t.memberExpression(arrayName, iterator, true))];

          if (originalExpression.params[1]) {
            forBodyDeclarations.push(t.variableDeclarator(originalExpression.params[1].name, iterator));
          }
          if (originalExpression.params[2]) {
            forBodyDeclarations.push(t.variableDeclarator(originalExpression.params[2].name, arrayName));
          }

          var forBody = [t.variableDeclaration("const", forBodyDeclarations), t.expressionStatement(name === "forEach" ? action : expr)];

          path.getStatementParent().insertBefore([t.variableDeclaration("const", [t.variableDeclarator(arrayName, path.node.callee.object)]), t.variableDeclaration("const", [t.variableDeclarator(funcName, path.node.arguments[0])])].concat(resArray, [t.forStatement(t.variableDeclaration("let", [t.variableDeclarator(iterator, t.numericLiteral(0))]), t.binaryExpression('<', iterator, t.memberExpression(arrayName, t.identifier('length'))), t.updateExpression('++', iterator), t.blockStatement(forBody))]));
