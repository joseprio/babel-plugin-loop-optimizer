    ...
    visitor: {
      ...
      CallExpression: function CallExpression(path) {
        ...
        if (checkName(path) && path.node.arguments.length === 1 && !t.isIdentifier(originalExpression) && (path.node.callee.property.name === 'forEach' || t.isExpression(originalExpression.body)) && originalExpression.params[0].type === 'Identifier') {
          ...
          var expr = t.isExpression(action) ? t.callExpression(t.memberExpression(resArrName, t.identifier("push")), [action]) : null;
          ...
          var forBody = [t.variableDeclaration("const", forBodyDeclarations)];
          if (name === 'forEach') {
            if (action.type === 'BlockStatement') {
              forBody.push.apply(forBody, _toConsumableArray(action.body));
            } else {
