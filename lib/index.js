    ...
    visitor: {
      ...
      CallExpression: function CallExpression(path) {
        ...
        if (checkName(path) && path.node.arguments.length === 1) {
          ...
          var call = t.callExpression(funcName, [t.memberExpression(arrayName, iterator, true), iterator, arrayName]);

          var resArray = name === "forEach" ? [] : [t.variableDeclaration("const", [t.variableDeclarator(resArrName, t.arrayExpression())])];

          var expr = t.callExpression(t.memberExpression(resArrName, t.identifier("push")), [call]);

          path.getStatementParent().insertBefore([t.variableDeclaration("const", [t.variableDeclarator(arrayName, path.node.callee.object)]), t.variableDeclaration("const", [t.variableDeclarator(funcName, path.node.arguments[0])])].concat(resArray, [t.forStatement(t.variableDeclaration("let", [t.variableDeclarator(iterator, t.numericLiteral(0))]), t.binaryExpression('<', iterator, t.memberExpression(arrayName, t.identifier('length'))), t.updateExpression('++', iterator), t.expressionStatement(name === "forEach" ? call : expr))]));
