    ...
    visitor: {
      ...
      CallExpression: function CallExpression(path) {
        ...
        if (checkName(path) && path.node.arguments.length === 1 && t.isExpression(path.node.arguments[0].body)) {
          ...
          path.getStatementParent().insertBefore([t.variableDeclaration("const", [t.variableDeclarator(arrayName, path.node.callee.object)])].concat(resArray, [t.forStatement(t.variableDeclaration("let", [t.variableDeclarator(iterator, t.numericLiteral(0))]), t.binaryExpression('<', iterator, t.memberExpression(arrayName, t.identifier('length'))), t.updateExpression('++', iterator), t.blockStatement(forBody))]));
